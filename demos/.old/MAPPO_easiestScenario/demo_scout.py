...
def _build_terrain_collections(water, buildings, forests):
    ...
    if bp:
        colls.append(PatchCollection(bp, facecolor=CLR_BUILDING, edgecolor='#aaaaaa',
...
def _render_frame(step, fire_map, b, n_quads,
    ...
    gs  = gridspec.GridSpec(n_rows, 2, width_ratios=[2, 1],
    ...
    ax_map.set_title(f"Step {step:04d}  –  Global overview", fontsize=11,
                     fontweight='bold', color='#222222')
    ax_map.set_xlabel('X [m]', fontsize=8, color='#555555')
    ax_map.set_ylabel('Y [m]', fontsize=8, color='#555555')
    ax_map.grid(True, color=CLR_GRID, linewidth=0.4, zorder=0)

    leg_h = []
    if terrain_collections:
        leg_h += [mpatches.Patch(color=CLR_FOREST,   label='Forest'),
    ...
    for qi in range(n_quads):
        ...
        if local_maps[qi] is not None:
            ...
            ax_c.text(0.5, 0.5, 'DEAD', ha='center', va='center',
                      fontsize=10, color='#999999', transform=ax_c.transAxes)
        fire_str = f"  fire={fire_sums[qi]:.2f}" if fire_sums[qi] > 0 else ""
        ax_c.set_title(f"{scout_labels[qi]}  (32×32){fire_str}",
                       fontsize=8.5, fontweight='bold', color=scout_colors[qi], pad=3)
        ax_c.set_xticks([]); ax_c.set_yticks([])

    # ── Stats panel ──────────────────────────────────────────────────
    ax_st = fig.add_subplot(gs[n_quads, 1])
    ax_st.set_facecolor(CLR_BG)
    ...
    for qi in range(n_quads):
        ...
        alt_str   = (f"{scout_positions[qi][2]:.0f} m"
                     if scout_alive[qi] and scout_positions[qi] is not None else "—")
        lines.append(f"S{qi} [{alive_str}]  R={total_rewards[qi]:+7.1f}  z={alt_str}")
    lines += ["", f"Step {step}"]
    ax_st.text(0.06, 0.95, "\n".join(lines), color='#222222', fontsize=8,
               va='top', transform=ax_st.transAxes, fontfamily='monospace')
    ax_st.set_title("Mission stats", fontsize=8.5, fontweight='bold',
                    color='#444444', pad=3)
...
def run_episode(env, scout_actor, seed, ep_num, n_quads, device,
    ...
    # ── Place multiple fires spread across the map ──────────────────
    b = env.map_bounds * 0.65          # keep fires inside 65% of half-extent
    fire_positions = []
    ...
    while len(fire_positions) < n_fires and attempts < 200:
        ...
        fx = _rnd.uniform(-b, b)
        fy = _rnd.uniform(-b, b)
        # Ensure fires are at least 150 m apart from each other
    ...
    for i, (fx, fy) in enumerate(fire_positions):
        radius = _rnd.uniform(6.0, 12.0)
        intensity = _rnd.uniform(0.6, 0.9)
        env.sim.environment.start_fire_at_position([fx, fy], intensity=intensity,
                                                   radius_m=radius)

    for _ in range(15):
        env.sim.environment.update_fire_simulation(real_dt=0.1)

    # ── Respawn scouts: assign each scout near a different fire ──────
    # If more scouts than fires, wrap around (two scouts near same fire)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scouts",      type=int,   default=2, choices=[2, 3, 4, 5])
